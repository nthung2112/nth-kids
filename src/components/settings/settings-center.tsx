import { Languages, Sparkles, Volume2, VolumeX, X } from "lucide-react";
...
export default function SettingsCenter({ onClose }: SettingsCenterProps) {
  ...
  const sectionTitle = (key: string, icon: string) => (
    <div className="mb-3 flex items-center gap-2">
      ...
      <h3 className="text-lg font-bold text-purple-800">{t(key)}</h3>
    </div>
  );
    ...
    >
      <Card className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto p-6">
        ...
        <div className="mb-5 text-center">
          <h2 className="text-2xl font-bold text-purple-800 sm:text-3xl">{t("settings.title")}</h2>
        ...
        <section className="mb-5">
          ...
          <Button
            onClick={() => {
              playClickSound();
              update({ reduceMotion: !prefs.reduceMotion });
            }}
