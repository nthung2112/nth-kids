...
import StickerReveal from "@/components/rewards/sticker-reveal";
...
const saveGallery = (entries: GalleryEntry[]): boolean => {
...
export default function ColoringGame() {
  ...
  const [stickerReveal, setStickerReveal] = useState<{
    unlock: RewardUnlock;
  ...
  const saveCurrentToGallery = () => {
    ...
    const persisted = saveGallery(next);
    if (!persisted) return;
    setGallery(next);
    setSavedFlash(true);
    setTimeout(() => setSavedFlash(false), 1500);
    const unlock = recordActivityCompletion({ activityId: "game:colors:coloring" });
    if (unlock) {
      const sticker = getStickerById(unlock.stickerId);
      if (sticker) {
        setStickerReveal({ unlock, sticker });
      }
    }
  };
  ...
  const renderTemplatePreview = (
    ...
    <svg width="100%" height="100%" viewBox="0 0 300 250" aria-hidden="true">
      {template.paths.map((path, index) => {
        ...
        return <path key={index} d={path} fill={fill} stroke="#333" strokeWidth="2" />;
      })}
    </svg>
  );
    ...
    <div className="mx-auto max-w-6xl">
      <div className="mb-3 flex flex-wrap items-center justify-end gap-2">
        ...
        >
          ...
          <span>
            {showReference ? t("games.coloring.hideReference") : t("games.coloring.showReference")}
          </span>
        </Button>
      ...
      <div className="grid grid-cols-1 gap-6 @5xl:grid-cols-3">
        <Card className="border-4 border-pink-300 bg-linear-to-br from-pink-100 to-purple-100 p-6">
          ...
          <div className="space-y-3">
            ...
            >
              <Save className="mr-2" />
              {savedFlash ? t("games.coloring.savedToGallery") : t("games.coloring.saveToGallery")}
            </Button>

            {stickerReveal && (
              <StickerReveal
                unlock={stickerReveal.unlock}
      ...
      <Card className="mt-6 border-4 border-purple-300 bg-white p-4">
        ...
        {gallery.length === 0 ? (
          <p className="text-center text-sm text-purple-500">{t("games.coloring.galleryEmpty")}</p>
        ) : (
